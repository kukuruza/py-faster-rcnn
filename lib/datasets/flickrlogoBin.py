...
class flickrlogoBin(imdb):
    ...
    def _load_pascal_annotation(self, index):
        ...
        overlaps = np.zeros((num_objs, 2), dtype=np.float32) # 'no-logo' & myclass
        ...
        for ix, obj in enumerate(objs):
            ...
            if obj.find('name').text.lower().strip() == self._myclass:
                ...
                gt_classes[ix] = 1  # 1 is the myclass index
                overlaps[ix, 1] = 1.0  # 1 is the myclass index
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        imagesetfile = os.path.join(
            ...
            'Main',
            self._image_set + '.txt')
        cachedir = os.path.join(self._devkit_path, 'annotations_cache_flickrlogo32')
        aps = []
        ...
        for i, cls in enumerate(self._classes):
            if cls == 'no-logo':
                continue
            filename = self._get_voc_results_file_template().format(cls)
            rec, prec, ap, prec_at_t = voc_eval(
